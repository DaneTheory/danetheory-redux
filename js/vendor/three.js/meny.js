	...
	create: function( options ) {
		return (function(){
			...
			var config = {
				...
				gradient: 'rgba(149, 159, 167, 0.5) 25%, rgba(64, 68, 70, 0.8) 100%)',
				mouse: true,
				touch: true,
			};
			...
			function setupPositions() {
				...
				contentsAngle = config.angle / -1.5;
                contentsOpacity = config.opacity / -2;
				...
				switch( config.position ) {
					...
					case POSITION_B:
						...
						contentsTransformOpened = 'translateY( -'+ config.opacity + config.height +'px ) rotateX( ' + -contentsAngle + 'deg )';
					...
					default:
						// Primary transform:
						menuTransformOrigin = '100% 50%';
						menuTransformClosed = 'translateX( -100% ) translateX( '+ config.overlap +'px ) scale( 1.04 ) rotateY( ' + -menuAngle + 'deg )';
